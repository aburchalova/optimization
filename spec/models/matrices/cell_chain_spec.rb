...
describe Matrices::CellChain do
  let(:with_cycle) { Matrices::CellChain.new(   [ [1, 0], [2, 0], [2, 1], [0, 1], [0, 2], [2, 2]  ]) }
  let(:without_cycle) { Matrices::CellChain.new([ [1, 0], [2, 0], [2, 1], [0, 1], [0, 2]          ]) }
  ...
  describe '#remove_rows_with_1_item!' do
    subject { with_cycle }
    let(:removed) { Matrices::CellChain.new([[2, 0], [2, 1], [0, 1], [0, 2], [2, 2]]) }
  ...
  describe '#remove_columns_with_1_item!' do
    subject { without_cycle }
    let(:removed) { Matrices::CellChain.new([[1, 0], [2, 0], [2, 1], [0, 1]]) }
  ...
  describe '#remove_rows_and_cols_with_1_item!' do
    ...
    context 'when cycle' do
      subject { with_cycle }
      let(:removed) { Matrices::CellChain.new([ [2, 1], [0, 1], [0, 2], [2, 2] ]) }
  ...
  describe '#cycle?' do
    it { with_cycle.should be_cycle  }
    it { without_cycle.should_not be_cycle  }
  end
